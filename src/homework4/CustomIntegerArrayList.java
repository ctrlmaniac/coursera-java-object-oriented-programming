...
public class CustomIntegerArrayList {
    ...
    public ArrayList<Integer> splice(int index, int num) {
        ...
        if (num == 0 ){
            ...
            int maxNum = this.arr.size() < num ? this.arr.size() : num;
            ArrayList<Integer> toReturn = new ArrayList<>();

            int current = 1;
            while(current <= maxNum) {
                toReturn.add(this.arr.get(index));
                this.arr.remove(index);
                current++;
            }
    ...
    public ArrayList<Integer> splice(int index, int num, int[] otherArray) {
        ArrayList<Integer> spliced = this.splice(index, num);

        int currentIndex = index;
        for (int i = 0; i < otherArray.length; i++) {
            this.add(currentIndex, otherArray[i]);
            currentIndex++;
        }
